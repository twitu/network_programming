...
void* event_generator(void* args) {
    ...
    // events slot 0 is always assigned to listener fd
    ...
    while (1) {
        ...
        for (int i = 0; i < event_count; i++) {
            ...
            if (events[i].data.fd == listen_fd) {
                add_event(listen_fd, ACCEPT_EVENT);
                #ifdef DEBUG
                print_tail_event();
                #endif
            } else {
                if (events[i].events == EPOLLIN) add_event(events[i].data.fd, READ_EVENT);
                else if (events[i].events == EPOLLOUT) add_event(events[i].data.fd, WRITE_EVENT);
                #ifdef DEBUG
                print_tail_event();
                #endif
            }
